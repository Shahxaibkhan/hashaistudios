...
export default function CarPectNewVehiclePage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true)
    try {
      const res = await fetch('/api/carpect/vehicles', {
        method: 'POST',
  ...
  return (
    <div>
      <div className="flex items-center gap-4 mb-8">
        <Link href="/carpect/vehicles" className="text-gray-400 hover:text-gray-600 transition-colors">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <select required value={form.make} onChange={f('make')}
              ...
              <div>
                ...
                <input type="text" required value={form.model} onChange={f('model')}
                  className="w-full px-3.5 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="number" required min={1980} max={2026} value={form.year} onChange={f('year')}
                  className="w-full px-3.5 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">Color *</label>
                <input type="text" required value={form.color} onChange={f('color')}
                  className="w-full px-3.5 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="text" required value={form.licensePlate} onChange={f('licensePlate')}
                  className="w-full px-3.5 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                  placeholder="LHR-5521" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">VIN</label>
                <input type="text" value={form.vin} onChange={f('vin')}
                  className="w-full px-3.5 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
            ...
            <button type="submit" disabled={loading}
              ...
              {loading ? <><Loader2 className="w-4 h-4 animate-spin" /> Adding...</> : 'Add Vehicle'}
            </button>
          </form>
        </div>
      </div>
