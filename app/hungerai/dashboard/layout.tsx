...
export default function DashboardLayout({ children }: DashboardLayoutProps) {
  ...
  const navItems = [
    ...
    { href: "/hungerai/dashboard/qr" as const, label: "QR Codes", icon: QrIcon },
  ];

  return (
    <div className="min-h-screen flex flex-col bg-[var(--hai-bg-secondary)]">
      {/* Header */}
      <header className="no-print sticky top-0 z-30 bg-[var(--hai-bg-card)] border-b border-[var(--hai-border)] shadow-sm">
            ...
            <div className="hai-nav">
              {navItems.map((item) => {
                ...
                return (
                  <Link
                    key={item.href}
                    href={item.href as "/hungerai/dashboard" | "/hungerai/dashboard/orders" | "/hungerai/dashboard/menu" | "/hungerai/dashboard/qr"}
      ...
      {/* Footer */}
      <footer className="no-print bg-[var(--hai-bg-card)] border-t border-[var(--hai-border)]">
...
function QrIcon({ className }: { className?: string }) {
  return (
    <svg className={className} fill="none" stroke="currentColor" viewBox="0 0 24 24">
      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4 4h6v6H4V4zm10 0h6v6h-6V4zM4 14h6v6H4v-6zm10 2h2m2 0h2m-6 4h2m2-4v4m2-4v4" />
    </svg>
  );
}
