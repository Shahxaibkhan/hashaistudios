...
export async function POST(req: Request) {
  ...
  const postInspection = await carpectPrisma.inspection.findFirst({
  ...
  const preInspection = await carpectPrisma.inspection.findFirst({
  ...
  await carpectPrisma.inspection.update({ where: { id: postInspectionId }, data: { status: 'IN_PROGRESS' } })
  ...
  await carpectPrisma.$transaction([
    carpectPrisma.damage.deleteMany({ where: { inspectionId: postInspectionId } }),
    ...allNewDamages.map(d =>
